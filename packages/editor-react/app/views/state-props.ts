 ...
 * has no editing leaf state yet, so the editor applies this inline. Uses the
 * permanent chrome --hairline and the generated --sdn-swatch-active token.
 */
export const EDITING_OUTLINE = "var(--hairline) solid var(--sdn-swatch-active)"

/**
 * The generated input's activated state paints a border. Sidebar rows use
 ...
 * applied as an inline style (see {@link EDITING_OUTLINE}) rather than a
 * global class patch.
 */
 ...
 * spread overwrites `className` and `style`, which silently drops one leaf
 * state when two apply at once (for example a row that is both editing and an
 * override). This concatenates every `className`, spreads `style` objects
 ...
 * forwards the class onto every leaf ref it drives. In the editor sidebars
 * activated is tint-only: the generated input state also paints a border,
 * which is withheld here so only edit mode outlines a row. Spread the result
 * into each leaf entry of `seldonRefs`.
 */
