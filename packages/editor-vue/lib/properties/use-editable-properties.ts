...
import { ValueType, type Value } from "@seldon/core"
 ...
 * subset of the React properties inspector.
 */
export function useEditableProperties(
  ...
  return computed(() => {
    ...
    const orderedKeys = Object.keys(properties.value)
    ...
    for (const key of orderedKeys) {
      const entry = properties.value[key]
      const meta = PROPERTY_DISPLAY_META[key]
      const category = meta.displayCategory
      const canReset = overrideKeys.value.has(key)
      ...
      if (isAtomicEntry(entry)) {
        pushRow(category, {
          ...
          control: buildControl(key, entry, workspace.value),
          isFacet: false,
          isHeader: false,
          canReset,
        })
