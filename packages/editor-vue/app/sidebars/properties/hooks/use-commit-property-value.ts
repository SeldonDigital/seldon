...
export function useCommitPropertyValue(deps: CommitDeps) {
  ...
  function resetProperty(
    ...
    dispatch({
      ...
      payload: { nodeId, propertyKey, subpropertyKey, layerIndex },
    } as never)
  }
  ...
  function reset(): void {
    ...
    if (property.isSubProperty && parsed.kind === "layered-facet") {
      resetProperty(subject.id, parsed.root, parsed.facet, parsed.index)
    } else if (property.isSubProperty && parsed.kind === "facet") {
      resetProperty(subject.id, parsed.root, parsed.facet)
    } else if (parsed.kind === "layered-parent") {
      resetProperty(subject.id, parsed.root, undefined, parsed.index)
    } else {
      resetProperty(subject.id, property.key)
    }
    deps.onDone()
  }
