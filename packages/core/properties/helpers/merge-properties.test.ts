...
describe("mergeProperties", () => {
  it("adds keys from the patch onto the base", () => {
    const result = mergeProperties(props({ opacity: exact(1) }), props({ color: exact("#fff") }))

    expect(result).toEqual({ opacity: exact(1), color: exact("#fff") })
  })

  it("overrides an atomic value with the patch", () => {
    const result = mergeProperties(props({ opacity: exact(1) }), props({ opacity: exact(2) }))

    expect(result.opacity).toEqual(exact(2))
  })

  it("skips an EMPTY patch over an existing value", () => {
    const result = mergeProperties(props({ color: exact("#fff") }), props({ color: empty() }))
  ...
  it("merges layered paint stacks by slot", () => {
    ...
    expect(result.background).toEqual([{ color: exact("A") }, { color: exact("C") }])
  })
