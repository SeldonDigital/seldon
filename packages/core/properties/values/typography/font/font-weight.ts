import type { Theme, ThemeFontWeightKey } from "../../../../themes/types"
...
export const fontWeightSchema: PropertySchema = {
  ...
  validation: {
    ...
    inherit: () => true,
    exact: (value: unknown) =>
      typeof value === "number" && Number.isFinite(value) && value >= 100 && value <= 900,
    themeOrdinal: (value: unknown, theme?: Theme) => {
      if (!theme || typeof value !== "string") return false

      return (Object.keys(theme.fontWeight) as string[]).some((id) => value === `@fontWeight.${id}`)
    },
  },
