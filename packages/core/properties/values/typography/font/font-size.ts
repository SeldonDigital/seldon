...
export const fontSizeSchema: PropertySchema = {
  name: "fontSize",
  description: "Sets text size from the theme scale, rem or px lengths, or a computed rule.",
  ...
  validation: {
    ...
    themeOrdinal: (value: unknown, theme?: Theme) => {
      ...
      return (Object.keys(theme.fontSize) as string[]).some((id) => value === `@fontSize.${id}`)
    },
  },
  themeOrdinalKeys: (theme: Theme) =>
    (Object.keys(theme.fontSize) as string[]).map((id) => `@fontSize.${id}` as ThemeFontSizeKey),
