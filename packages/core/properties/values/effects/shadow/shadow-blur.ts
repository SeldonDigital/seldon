...
import type { Theme, ThemeBlurKey } from "../../../../themes/types"
...
export type ShadowBlurValue = EmptyValue | PixelValue | RemValue | ShadowBlurThemeValue

/** Validates stored shadow blur values. */
export const shadowBlurSchema: PropertySchema = {
  name: "shadowBlur",
  description: "Sets how soft the shadow edge is using exact lengths or theme blur steps.",
  ...
  validation: {
    ...
    themeOrdinal: (value: unknown, theme?: Theme) => themeTokenRefIsValid(value, theme, "blur"),
  },
  themeOrdinalKeys: (theme: Theme) => Object.keys(theme.blur).map((id) => `@blur.${id}`),
}
