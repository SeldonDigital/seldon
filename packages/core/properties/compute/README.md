...
- `basedOn` paths use `#` for the current node, `#parent.` for the immediate parent, and `#self.` for the current node with a parent fallback. Runtime paths use dot segments such as `background.0.color`.
