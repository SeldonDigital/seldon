...
describe("getBasedOnValue", () => {
  it("resolves a #self path on the node's own properties", () => {
    expect(getBasedOnValue("#self.buttonSize", ctx({ buttonSize: exact(16) }))).toEqual(
      exact(16),
    )
  })

  it("resolves a bare path with no anchor prefix", () => {
    expect(getBasedOnValue("buttonSize", ctx({ buttonSize: exact(16) }))).toEqual(
      exact(16),
    )
  })
