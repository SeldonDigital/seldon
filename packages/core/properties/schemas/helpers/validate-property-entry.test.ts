...
describe("collectPropertyValueErrors", () => {
  it("returns no errors for a well-formed atomic theme ref", () => {
    expect(collectPropertyValueErrors("color", swatch("@swatch.primary"), defaultTheme)).toEqual([])
  })
  ...
  it("rejects a flat value on a layered paint property", () => {
    const errors = collectPropertyValueErrors("background", swatch("@swatch.primary"), defaultTheme)
  ...
  it("validates a dotted layered path against its flattened schema", () => {
    expect(
      collectPropertyValueErrors("background.0.color", swatch("@swatch.primary"), defaultTheme),
    ).toEqual([])
  ...
  it("treats EMPTY as valid on any property", () => {
    expect(
      collectPropertyValueErrors("color", { type: ValueType.EMPTY, value: null }, defaultTheme),
    ).toEqual([])
  })
