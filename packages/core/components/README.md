...
A Heading, Paragraph, or List Item is a wrapper element. Heading renders as `h1` through `h6`. Paragraph renders as `p`. List Item renders as `li`, `dt`, or `dd`. Each has a Text child. Text editing writes that child's `content` and `runs`. A run is an inline `span`, `b`, `em`, or `strong` stored on the primitive. It is not a workspace child. Combined bold and italic uses one tag plus the other axis on `font.weight` or `font.style`. Return inserts a sibling Text when the parent can take one. Return on a lone primitive commits and exits.
