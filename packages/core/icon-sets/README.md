...
The editor loads all three vendor data files eagerly on first icon render, a few MB on desktop and bounded by the per-set cap. The icon set board renders at most `MAX_RENDERED_BOARD_ICONS` previews and shows an overflow notice past that, so enabling a whole large set cannot mount thousands of component trees.
