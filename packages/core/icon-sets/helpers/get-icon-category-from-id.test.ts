...
describe("iconBelongsToIconSet", () => {
  it("matches an icon id against its set prefix", () => {
    expect(iconBelongsToIconSet("material-add" as IconId, "google-material" as IconSetId)).toBe(
      true,
    )
  })

  it("rejects an icon id from another set", () => {
    expect(iconBelongsToIconSet("lucide-x" as IconId, "google-material" as IconSetId)).toBe(false)
  })
})
