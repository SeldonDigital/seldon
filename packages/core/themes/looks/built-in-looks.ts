...
import type { ThemeBorderKey, ThemeFontKey, ThemeShadowKey } from "../types/theme-reference-keys"
...
const CLEARED_LOOK_SECTIONS: readonly ClearedLookSection[] = ["shadow", "border", "font"] as const
...
function isClearedLookSection(section: BuiltInLookSection): section is ClearedLookSection {
...
export function getBuiltInLookId(section: BuiltInLookSection): string | null {
  return isClearedLookSection(section) ? BUILT_IN_LOOK_DEFINITIONS[section].id : null
}
...
export function getBuiltInLookToken(section: BuiltInLookSection): string | null {
  return isClearedLookSection(section) ? BUILT_IN_LOOK_DEFINITIONS[section].token : null
}

export function isBuiltInLookSection(propertyKey: string): propertyKey is BuiltInLookSection {
...
export function injectBuiltInLooks<T extends StockTheme | ComputedTheme>(theme: T): T {
