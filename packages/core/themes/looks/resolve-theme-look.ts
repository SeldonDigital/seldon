...
export function listThemeLookIds(theme: Theme, section: BuiltInLookSection): string[] {
...
export function getThemeLookPickerToken(propertyKey: string, lookId: string): string {
  const sectionKey = getBuiltInLookSectionForPropertyKey(propertyKey)
...
export function isBuiltInClearedLookToken(propertyKey: string, presetValue: unknown): boolean {
  const sectionKey = getBuiltInLookSectionForPropertyKey(propertyKey)
...
export function isThemeLookPresetSchemaName(schemaName: string): boolean {
  return BUILT_IN_LOOK_SECTIONS.some((section) => schemaName === `${section}Preset`)
}

export function resolveBuiltInLookApplyName(propertyKey: string, preset: string): string | null {
  const sectionKey = getBuiltInLookSectionForPropertyKey(propertyKey)
