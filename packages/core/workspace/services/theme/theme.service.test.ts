...
describe("WorkspaceThemeService.getNextCustomTokenIdForTheme", () => {
  it("returns custom1 for an empty section and the next slot once seeded", () => {
    ...
    expect(service.getNextCustomTokenIdForTheme(base, "theme-seldon-copy", "swatch")).toBe(
      "custom1",
    )
    ...
    expect(service.getNextCustomTokenIdForTheme(seeded, "theme-seldon-copy", "swatch")).toBe(
      "custom2",
    )
  })
})
