...
import type { SandboxRect } from "../../helpers/nodes/sandbox"
import type { Middleware, Workspace } from "../../types"
...
const validators = {
  ...
  allNodeTemplateTargetsExist: (workspace: Workspace) => {
    ...
    Object.values(nodes).forEach((node) => {
      ...
      check(nodes[link.nodeId], ErrorMessages.missingInstance(node.id, link.nodeId))
    })
  },
  ...
  oneDefaultVariantPerBoard: (workspace: Workspace) => {
    ...
    Object.values(workspace.boards).forEach((board) => {
      ...
      const boardKey =
        getBoardKey(workspace, board) ?? ("catalogId" in board ? board.catalogId : "")
  ...
  authoredRootsAreValid: (workspace: Workspace) => {
    ...
    for (const board of Object.values(workspace.boards)) {
      ...
      check(board.variants.length > 0, "Authored board is missing its authored root.")
      ...
      for (const ref of playground.variants) {
        ...
        for (const other of placed) {
          if (sandboxesOverlap(rect, other.rect)) {
            throw new Error(`Sandboxes ${other.id} and ${ref.id} overlap in playground ${key}.`)
          }
        }
...
const VERIFICATION_CHECKS: Array<[string, (workspace: Workspace) => void]> = [
  ...
  ["All node template targets exist", (w) => validators.allNodeTemplateTargetsExist(w)],
 ...
 */
export const workspaceVerificationMiddleware: Middleware = (next) => (workspace, action) => {
  const nextWorkspace = next(workspace, action)
  const shouldLog = process.env.NODE_ENV === "development" && isWorkspaceLoggingEnabled()
