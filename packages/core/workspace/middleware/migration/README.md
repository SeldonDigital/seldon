...
Repair steps run on every load, regardless of stored version. They cover stock theme and icon set renames that must also reach files already stamped at the current version, board order, duplicate node refs, and missing export settings. Each repair guards itself and only rewrites the references it matches, so it is safe to re-run.
