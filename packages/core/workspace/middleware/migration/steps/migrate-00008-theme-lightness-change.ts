...
/**
 * v8: backfill `colorHarmony.parameters.lightnessChange`.
 *
 * Stock themes now author the parameter, so a theme entry whose template chain
 * reaches a known stock theme resolves it by deep merge and needs no change.
 * This step stamps `lightnessChange: 0` into an entry's overrides only when the
 * chain cannot supply a value.
 */
...
/** True when the entry cannot resolve `lightnessChange` from its template chain. */
function entryNeedsStamp(entry: EntryTheme, themes: Record<string, EntryTheme>): boolean {
  ...
  return !chainOverridesSupply(entry, themes, "lightnessChange")
}

function migrationApplies(workspace: Workspace): boolean {
  return Object.values(workspace.themes).some((entry) => entryNeedsStamp(entry, workspace.themes))
}

export function migrateV8ThemeLightnessChange(workspace: Workspace): Workspace {
  ...
  for (const entry of Object.values(themes)) {
    if (!entryNeedsStamp(entry, themes)) continue
