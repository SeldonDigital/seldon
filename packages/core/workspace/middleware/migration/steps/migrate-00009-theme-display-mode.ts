...
function overrideParameters(entry: EntryTheme, group: string): Record<string, unknown> | undefined {
...
export function migrateV9ThemeDisplayMode(workspace: Workspace): Workspace {
  ...
  for (const entry of Object.values(next.themes)) {
    ...
    const displayMode = (overrides.displayMode ??= {}) as Record<string, unknown>
    const displayModeParameters = (displayMode.parameters ??= {}) as Record<string, unknown>
