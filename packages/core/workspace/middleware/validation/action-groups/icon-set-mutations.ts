...
export function validateIconSetMutation(workspace: Workspace, action: Action): void {
  switch (action.type) {
...
function assertNonEmptyOverridePath(action: Action): void {
  ...
  if (typeof path !== "string" || path.length === 0) {
    throw new WorkspaceValidationError("Icon set override path must be a non-empty string", action)
  }
}
...
function assertSubcategoryPresetPayload(action: Action): void {
  ...
  if (typeof payload.subcategory !== "string" || payload.subcategory.length === 0) {
    throw new WorkspaceValidationError("Icon set subcategory must be a non-empty string", action)
  }

  if (typeof payload.preset !== "string" || !SUBCATEGORY_PRESETS.has(payload.preset)) {
...
function assertIconSetDeletable(workspace: Workspace, id: string, action: Action): void {
  const entry = workspace["icon-sets"][id]

  if (entry && isEntryIconSetDefault(entry)) {
    throw new WorkspaceValidationError("Cannot remove default icon set entry", action)
  }
}
