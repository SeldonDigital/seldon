...
import { boardValidators, propertyValidators, themeValidators } from "../validators"
...
function assertBoardHasAllowedKind(
  ...
  if (!allowed.includes(board.type)) {
    throw new WorkspaceValidationError("That update does not apply to this board type.", action)
  }
}
...
function resolvePropertySchemaId(
  ...
  if (board && isAuthoredBoard(board)) {
    ...
    const rootNode = rootId ? (workspace.nodes[rootId] as EntryNode | undefined) : undefined
...
const AUTHOR_BOARDS = ["component", "theme"] as const satisfies ReadonlyArray<Board["type"]>
...
const COMPONENT_BOARDS = ["component"] as const satisfies ReadonlyArray<Board["type"]>

export function validateBoardMetadata(workspace: Workspace, action: Action): void {
  switch (action.type) {
    ...
    case "reset_board_license":
      assertBoardHasAllowedKind(workspace, action.payload.boardKey, action, LICENSE_BOARDS)

      return
    case "set_board_author":
    case "reset_board_author":
      assertBoardHasAllowedKind(workspace, action.payload.boardKey, action, AUTHOR_BOARDS)

      return
    case "set_board_credentials":
    case "reset_board_credentials":
      assertBoardHasAllowedKind(workspace, action.payload.boardKey, action, CREDENTIALS_BOARDS)

      return
    case "set_board_preview":
    case "reset_board_preview":
      assertBoardHasAllowedKind(workspace, action.payload.boardKey, action, PREVIEW_BOARDS)
    ...
    case "set_component_properties": {
      ...
      propertyValidators.values(action.payload.properties, workspace, board?.componentTheme)
    ...
    case "reset_component_board":
      assertBoardHasAllowedKind(workspace, action.payload.boardKey, action, COMPONENT_BOARDS)

      return
    case "apply_component_properties_to_all_boards":
      assertBoardHasAllowedKind(workspace, action.payload.sourceBoardKey, action, COMPONENT_BOARDS)
