...
export function validateFontCollectionMutation(workspace: Workspace, action: Action): void {
  switch (action.type) {
...
function assertNonEmptyOverridePath(action: Action, label: string): void {
  ...
  if (typeof path !== "string" || path.length === 0) {
    throw new WorkspaceValidationError(`${label} path must be a non-empty string`, action)
  }
}
...
function assertFamilyVariantPayload(action: Action): void {
  ...
  if (typeof payload.variant !== "string" || payload.variant.length === 0) {
    throw new WorkspaceValidationError("Family variant must be a non-empty string", action)
  }
...
function assertFamilyPresetPayload(action: Action): void {
  ...
  if (typeof payload.preset !== "string" || !FAMILY_PRESETS.has(payload.preset)) {
    throw new WorkspaceValidationError('Family preset must be "all" or "none"', action)
  }
}

/** Rejects deleting the default font collection entry. */
function assertFontCollectionDeletable(workspace: Workspace, id: string, action: Action): void {
  const entry = workspace["font-collections"][id]

  if (entry && isEntryFontCollectionDefault(entry)) {
    throw new WorkspaceValidationError("Cannot remove default font collection entry", action)
  }
}

export function validateAddFontCollectionCustomFamily(workspace: Workspace, action: Action): void {
  ...
  if (!payload.name?.trim()) {
    throw new WorkspaceValidationError("Custom font family name is required", action)
  }
}
...
export function validateRemoveFontCollectionCustomFamily(
  ...
  fontCollectionEntryValidators.isVariant(workspace, payload.fontCollectionId)
  fontCollectionEntryValidators.customFamilyExists(workspace, payload.fontCollectionId, payload.key)
}
