...
export function validateSetThemeScaleSlot(workspace: Workspace, action: Action): void {
  ...
  if (!value || (value.kind !== "modulated" && value.kind !== "exact")) {
    throw new WorkspaceValidationError("Scale slot value must be modulated or exact", action)
  }
  ...
  if (value.kind === "exact") {
    ...
    if (unit !== Unit.PX && unit !== Unit.REM) {
      throw new WorkspaceValidationError("Exact scale slot unit must be px or rem", action)
    }

    if (typeof numeric !== "number" || !Number.isFinite(numeric)) {
      throw new WorkspaceValidationError("Exact scale slot value must be a finite number", action)
    }
  }
}

export function validateSetThemeCustomTokenName(workspace: Workspace, action: Action): void {
  const payload = action.payload as {
    themeId: string
    section: string
    key: string
