...
import type { InstanceId, VariantId } from "../types"
...
export const ErrorMessages = {
  ...
  componentNotFoundForVariant: (id: VariantId) => `Board not found for variant ${id}.`,
  tooManyDefaultVariants: (id: ComponentId) => `Board ${id} has more than one default variant.`,
  ...
  variantsInUseForReset: (usages: { usingBoardLabel: string; variantLabel: string }[]) => {
    const detail = usages
  ...
  themeLabelNotUnique: (label: string) =>
    `A theme that is called ${label} already exists. Please choose another name.`,
  refNotUnique: (ref: string) =>
    `The reference "${ref}" is already in use. Please choose another reference.`,
  defaultVariantNotFound: (id: ComponentId) => `Default variant not found for component ${id}.`,
  ...
  nodeNotVariant: (id: InstanceId | VariantId) => `Node ${id} is not a variant.`,
  nodeNotInstance: (id: InstanceId | VariantId) => `Node ${id} is not an instance but a variant.`,
  childNotAllowed: (id: InstanceId | VariantId) => `Node ${id} cannot have children.`,
  parentNotFound: (id: InstanceId | VariantId) => `Parent node not found for node ${id}.`,
  ...
  cyclicComponentTree: (id: string) => `Component tree contains a cycle at node ${id}.`,
  danglingVariant: (id: VariantId) => `Variant ${id} is not used in any board.`,
  danglingChildNode: (id: InstanceId) => `Child node ${id} is not used in any node.`,
  instanceMissingOrigin: (id: InstanceId) =>
    `Instance ${id} is missing a valid origin classification.`,
  missingInstance: (id: InstanceId | VariantId, instance: InstanceId | VariantId) =>
    `Node ${id} references ${instance} which does not exist.`,
  duplicateIds: (ids: string[]) => `Duplicate IDs found in workspace: ${ids.join(", ")}.`,
  ...
  cannotMoveToDefaultPosition: () => "Cannot move a variant to the default position",
  cannotAddSelfAsInstance: () => "Cannot add self as an instance to self",
  cannotAddChild: (component: string) => `${component} does not allow adding children`,
  invalidParentChildRelationship: (parentComponent: string, childComponent: string) =>
