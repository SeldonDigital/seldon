...
The `reducer` switch has no default branch. Validation rejects unknown action types before the reducer runs. Some actions return the workspace unchanged: `transcript_add_message` and every `stubs_*` action. The `add_component_and_insert_default_instance` action has no single handler. The reducer composes it from `addComponent` and `insertVariantInstance`.
