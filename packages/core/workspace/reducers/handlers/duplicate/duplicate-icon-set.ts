...
export function duplicateIconSet(
  ...
  return produce(workspace, (draft) => {
    const draftEntry = draft["icon-sets"][payload.iconSetId] as EntryIconSet | undefined

    if (!draftEntry) return

    const entry = (isDraft(draftEntry) ? current(draftEntry) : draftEntry) as EntryIconSet

    const boardKey = iconSetBoardKeyFromEntryId(payload.iconSetId)

    if (!boardKey) return

    const newId = payload.newIconSetId ?? formatEntryId("icon-set", boardKey, randomSuffix())
