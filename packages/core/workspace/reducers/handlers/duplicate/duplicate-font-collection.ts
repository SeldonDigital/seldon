...
/**
 * Clones a `font-collections` entry, points its template at the source, and appends it to the
 * owning font collection board.
 */
export function duplicateFontCollection(
  ...
  return produce(workspace, (draft) => {
    ...
    const isCollectionBoard = board?.type === "font-collection"

    const base = isCollectionBoard ? board.label : entry.label
    const existing = new Set<string>()
    if (isCollectionBoard && board.variants) {
