...
import type { ComponentBoard, ComponentTreeRef, ExtractPayload } from "../../../../index"
...
describe("duplicateFontCollection", () => {
  it("clones the default entry into a variant and appends it to the board", () => {
    ...
    expect(
      (next.boards.system as ComponentBoard).variants.map((r: ComponentTreeRef) => r.id),
    ).toContain("fc-dup")
  })
  ...
  it("is a no-op for an unknown id", () => {
    ...
    expect(duplicateFontCollection({ fontCollectionId: "missing" } as never, ws)).toBe(ws)
  })
})
...
describe("duplicateIconSet", () => {
  it("clones the default entry into a variant and appends it to the board", () => {
    ...
    expect(
      (next.boards.seldonIcons as ComponentBoard).variants.map((r: ComponentTreeRef) => r.id),
    ).toContain("is-dup")
  })
...
describe("duplicateNode", () => {
  const componentWorkspace = () =>
    addComponent({ boardKey: ComponentId.BUTTON } as ExtractPayload<"add_component">, empty())
  ...
  it("duplicates a default variant root into a new board variant", () => {
    ...
    const next = duplicateNode({ nodeId: rootId } as ExtractPayload<"duplicate_node">, ws)

    expect((next.boards[ComponentId.BUTTON] as ComponentBoard).variants.length).toBe(before + 1)
  })
})
