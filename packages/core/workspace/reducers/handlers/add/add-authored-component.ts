...
export function addAuthoredComponent(
  ...
  const rootComponentId = payload.rootKind === "frame" ? ComponentId.FRAME : ComponentId.CONTAINER

  return produce(workspace, (draft) => {
    const existing = Object.values(draft.boards)
    const maxOrder =
      existing.length > 0 ? Math.max(...existing.map((board) => getBoardOrder(board))) : -1
    ...
    const board: AuthoredComponentBoard = {
      ...
      label: workspaceMutationService.getInitialAuthoredComponentLabel(payload.name),
      intent: payload.intent,
      tags: payload.tags,
      componentTheme: WORKSPACE_EDITABLE_THEME_ENTRY_ID,
      componentProperties: getInitialBoardComponentProperties("authored-component"),
      variants: [{ id: rootId }],
    }
