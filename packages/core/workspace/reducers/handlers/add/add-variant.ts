 ...
 */
export function addVariant(payload: ExtractPayload<"add_variant">, workspace: Workspace) {
  ...
  if (payload.ensureDescendantComponents) {
    ...
    for (const componentId of components) {
      if (!nextWorkspace.boards[componentId as ComponentId]) {
        nextWorkspace = addComponent({ boardKey: componentId as ComponentId }, nextWorkspace)
      }
    }
  }
