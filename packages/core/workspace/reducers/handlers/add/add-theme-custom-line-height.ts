...
/**
 * Appends a custom lineHeight token (exact unitless number) to a variant theme entry's
 * `overrides.lineHeight` bag. No-ops when the entry is missing or marked `type: "default"`.
 */
export function addThemeCustomLineHeight(
  payload: ExtractPayload<"add_theme_custom_lineHeight">,
  workspace: Workspace,
): Workspace {
  return produce(workspace, (draft) => {
    ...
    appendCustomToken(entry, "lineHeight", id, cell)
  })
}
