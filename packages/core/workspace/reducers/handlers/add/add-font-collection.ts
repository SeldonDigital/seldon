...
export function addFontCollection(
  ...
  return produce(workspace, (draft) => {
    ...
    const stock = STOCK_FONT_COLLECTIONS_BY_ID[boardKey as FontCollectionTemplateId]
    const label = stock
      ? stock.metadata.name
    ...
    // A collection enables only the families it declares as default-on. Each
    // such family is written as an explicit `All`; every other family is left
