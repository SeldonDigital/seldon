...
function readFamilies(entry: EntryFontCollection): Record<string, FontFamilyEntry> {
  const current = (entry.overrides as Record<string, unknown>)[FONT_COLLECTION_FAMILY_SECTION]
...
export function removeCustomFamily(entry: EntryFontCollection, id: string): void {
  const families = readFamilies(entry)
