...
const withTwoLayers = addNodeLayer(
  { nodeId, property: "background", seed: { tag: "extra" } } as ExtractPayload<"add_node_layer">,
  base,
)
...
describe("removeNodeLayer", () => {
  it("removes a non-base layer and shortens the stack", () => {
    ...
    const next = removeNodeLayer(
      { nodeId, property: "background", index: length - 1 } as ExtractPayload<"remove_node_layer">,
      withTwoLayers,
    )
  ...
  it("refuses to remove the base layer at index 0", () => {
    expect(
      removeNodeLayer(
        { nodeId, property: "background", index: 0 } as ExtractPayload<"remove_node_layer">,
        withTwoLayers,
      ),
    ).toBe(withTwoLayers)
  })
  ...
  it("is a no-op for a missing node", () => {
    expect(
      removeNodeLayer(
        { nodeId: "ghost", property: "background", index: 1 } as ExtractPayload<"remove_node_layer">,
        withTwoLayers,
      ),
    ).toBe(withTwoLayers)
  })
...
describe("setNodeLayerKind", () => {
  ...
  it("is a no-op for a kind with no seed", () => {
    expect(
      setNodeLayerKind(
        { nodeId, property: "background", kind: "bogus" } as ExtractPayload<"set_node_layer_kind">,
        base,
      ),
    ).toBe(base)
  })
