...
describe("setFontCollectionFamilyVariant", () => {
  it("enables a variant under its family slot", () => {
    ...
    const overrides = next["font-collections"][fontCollectionId]!.overrides as Record<
      string,
  ...
  it("drops the slot when its last variant is disabled", () => {
    ...
    const overrides = disabled["font-collections"][fontCollectionId]!.overrides as Record<
      string,
      Record<string, Record<string, unknown>>
    >
