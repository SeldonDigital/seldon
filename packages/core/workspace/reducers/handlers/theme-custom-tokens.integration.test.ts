...
const removePayload = (themeId: string) => ({ themeId, key: "custom1" }) as never

const section = (ws: Workspace, key: string) =>
  ((ws.themes[variantThemeId]!.overrides as Record<string, unknown>)[key] ?? {}) as Record<
    string,
    unknown
  >
...
const REMOVE_SECTIONS = ADD_SECTIONS as Exclude<ThemeCustomTokenSection, "swatch">[]

describe("addThemeCustomToken", () => {
  it.each(ADD_SECTIONS)("%s appends a custom slot to a variant theme", (key) => {
    const next = addThemeCustomToken(key, addPayload(variantThemeId), variantWorkspace())
  ...
  it.each(ADD_SECTIONS)("%s is a no-op for a default theme entry", (key) => {
    ...
    expect(addThemeCustomToken(key, addPayload(defaultThemeId), workspace)).toBe(workspace)
  })
})

describe("removeThemeCustomToken", () => {
  it.each(REMOVE_SECTIONS)("%s drops a custom slot from a variant theme", (key) => {
    const seeded = produce(variantWorkspace(), (draft) => {
      ;(draft.themes[variantThemeId]!.overrides as Record<string, unknown>)[key] = {
        custom1: { name: "seed" },
      }
    })
    const removed = removeThemeCustomToken(key, removePayload(variantThemeId), seeded)
  ...
  it.each(REMOVE_SECTIONS)("%s is a no-op for a default theme entry", (key) => {
    ...
    expect(removeThemeCustomToken(key, removePayload(defaultThemeId), workspace)).toBe(workspace)
  })
})
