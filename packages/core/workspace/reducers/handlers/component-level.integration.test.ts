...
describe("setComponentTheme", () => {
  it("sets the board's default theme reference", () => {
    const next = setComponentTheme({ boardKey, theme: "seldon" } as never, componentWorkspace())
...
describe("setComponentProperties / resetComponentProperty", () => {
  it("merges a property onto the board and clears it again", () => {
    ...
    const set = setComponentProperties({ boardKey, properties: opacity } as never, ws)

    expect(board(set).componentProperties.opacity).toMatchObject({
      type: ValueType.EXACT,
      value: 0.5,
    })
    const reset = resetComponentProperty({ boardKey, propertyKey: "opacity" } as never, set)
...
describe("applyComponentPropertiesToAllBoards", () => {
  it("returns a workspace and preserves the source board props", () => {
    ...
    const next = applyComponentPropertiesToAllBoards({ sourceBoardKey: boardKey } as never, ws)
...
describe("resetDefaultVariantToCatalog", () => {
  it("drops an ad-hoc override on the default variant root", () => {
    const ws = componentWorkspace()
    const id = rootId(ws)
    const dirtied = setNodeProperties({ nodeId: id, properties: opacity } as never, ws)
    const reset = resetDefaultVariantToCatalog({ defaultVariantRootId: id } as never, dirtied)

    expect((reset.nodes[id]!.overrides as { opacity?: unknown }).opacity).toBeUndefined()
  })
  ...
  it("is a no-op for a non-default-variant node", () => {
    ...
    expect(
      resetDefaultVariantToCatalog({ defaultVariantRootId: instanceId(ws) } as never, ws),
    ).toBe(ws)
  })
})
...
describe("resetInstanceToSource / resetInstanceToOriginal", () => {
  it("clears instance subtree overrides", () => {
    ...
    const dirtied = setNodeProperties({ nodeId: instance, properties: opacity } as never, ws)
    const source = resetInstanceToSource(
      { instanceId: instance } as ExtractPayload<"reset_instance_to_source">,
      dirtied,
    )

    expect((source.nodes[instance]!.overrides as { opacity?: unknown }).opacity).toBeUndefined()
  })
  ...
  it("reverts an instance to its original template chain", () => {
    ...
    expect((original.nodes[instance]!.overrides as { opacity?: unknown }).opacity).toBeUndefined()
  })
})
