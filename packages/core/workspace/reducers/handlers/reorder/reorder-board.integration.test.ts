...
import type { ComponentBoard, EntryNode, ExtractPayload } from "../../../../index"
...
describe("reorderBoard", () => {
  it("moves a board to the front of the order", () => {
    ...
    const moved = reorderBoard({ boardKey, newIndex: 0 } as ExtractPayload<"reorder_board">, ws)
  ...
  it("is a no-op for an out-of-range index", () => {
    const ws = componentWorkspace()
    const result = reorderBoard({ boardKey, newIndex: -1 } as ExtractPayload<"reorder_board">, ws)
...
describe("reorderVariantInBoard", () => {
  ...
  it("is a no-op for a non-variant node", () => {
    const ws = componentWorkspace()
    const instanceId = Object.values(ws.nodes).find((n: EntryNode) => n.type === "instance")!
      .id as string
    const result = reorderVariantInBoard(
      {
