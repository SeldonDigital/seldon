...
const withUserVariant = (): { ws: Workspace; userVariantId: string } => {
  ...
  const before = (ws.boards[boardKey] as ComponentBoard).variants.map((v: ComponentTreeRef) => v.id)
...
describe("resetNodeLabel", () => {
  ...
  it("resets an instance label to its template-derived name", () => {
    ...
    const result = resetNodeLabel({ nodeId: instanceId } as ExtractPayload<"reset_node_label">, ws)
