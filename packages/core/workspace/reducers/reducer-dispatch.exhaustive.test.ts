...
function firstEntryIdOfType(map: Record<string, { type: string }>, type: string): string {
...
function findRemovableBoard(ids: ReadonlySet<string>, ws: Workspace, protectedKey: string): string {
  for (const id of ids) {
...
function buildBase() {
  ...
  ws = produce(ws, (draft) => {
    ...
    const fcOverrides = draft["font-collections"][FC_VARIANT_ID]!.overrides as Record<
      string,
      Record<string, unknown>
    >
  ...
  const removableTheme = findRemovableBoard(THEME_BOARD_CATALOG_IDS, ws, DEFAULT_THEME_BOARD_KEY)
...
const CASES: Array<[string, WorkspaceAction, Workspace?]> = [
  ...
  ["remove_board (font_collection)", act("remove_board", { boardKey: ids.removableFc })],
  ["remove_board (icon_set)", act("remove_board", { boardKey: ids.removableIconSet })],
  ["remove_board (theme)", act("remove_board", { boardKey: ids.removableTheme })],
  [
  ...
  ["paste_node_properties", act("paste_node_properties", { nodeId: ids.uv1Id, properties: {} })],
  [
    "reset_node_property",
