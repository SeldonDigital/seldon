...
function buildBase() {
  ...
  ws = dispatch(ws, act("add_custom_state", { key: "mystate", label: "My State" }))
  ...
  ws = dispatch(ws, act("add_node_layer", { nodeId: uv1Id, property: "background", seed: {} }))
  ws = dispatch(ws, act("add_node_layer", { nodeId: uv1Id, property: "background", seed: {} }))
  ...
  ws = produce(ws, (draft) => {
    ...
    for (const section of REMOVE_THEME_SECTIONS) {
      overrides[section] = { ...(overrides[section] ?? {}), custom1: { name: "seed" } }
    }
  ...
  const baseNoFc = dispatch(ws, act("remove_font_collection", { catalogId: removableFc }))
...
function removeThemeCustomCase(section: string): [string, WorkspaceAction] {
  return [
    ...
    act(`remove_theme_custom_${section}`, { themeId: THEME_VARIANT_ID, key: "custom1" }),
  ]
}
...
const CASES: Array<[string, WorkspaceAction, Workspace?]> = [
  ...
  ["set_workspace_last_update", act("set_workspace_last_update", { value: "2026-01-01" })],
  ...
  ["add_font_collection", act("add_font_collection", { catalogId: ids.addFcId }), baseNoFc],
  ...
  ["remove_font_collection", act("remove_font_collection", { catalogId: ids.removableFc })],
  ["remove_icon_set", act("remove_icon_set", { catalogId: ids.removableIconSet })],
  ["remove_theme", act("remove_theme", { boardKey: ids.removableTheme })],
  ["duplicate_playground", act("duplicate_playground", { sourcePlaygroundKey: "pg-1", newPlaygroundKey: "pg-dup" })],
  ["reorder_board", act("reorder_board", { boardKey: BOARD, newIndex: 0 })],
  [
    "reorder_variant_in_board",
    act("reorder_variant_in_board", { boardKey: BOARD, variantRootId: ids.uv1Id, newIndex: 1 }),
  ],

  // Component-level board edits.
  ["set_component_theme", act("set_component_theme", { boardKey: BOARD, theme: ids.themeDefaultId })],
  ["set_component_properties", act("set_component_properties", { boardKey: BOARD, properties: {} })],
  ["reset_component_property", act("reset_component_property", { boardKey: BOARD, propertyKey: ids.buttonPropKey })],
  ["apply_component_properties_to_all_boards", act("apply_component_properties_to_all_boards", { sourceBoardKey: BOARD })],

  // Board metadata writes.
  ["set_board_label", act("set_board_label", { boardKey: BOARD, label: "Btn" })],
  ["set_playground_label", act("set_playground_label", { playgroundKey: "pg-1", label: "PG" })],
  ["set_board_intent", act("set_board_intent", { boardKey: BOARD, intent: "i" })],
  ["set_board_tags", act("set_board_tags", { boardKey: BOARD, tags: ["t"] })],
  ["set_board_license", act("set_board_license", { boardKey: BOARD, license: { name: "MIT" } })],
  ["set_board_author", act("set_board_author", { boardKey: BOARD, author: "me" })],
  ["set_board_credentials", act("set_board_credentials", { boardKey: ids.fontCollectionBoardKey, credentials: { key: "v" } })],
  ["set_board_preview", act("set_board_preview", { boardKey: ids.fontCollectionBoardKey, preview: "url" })],
  ["set_board_editor_data", act("set_board_editor_data", { boardKey: BOARD, editorData: { x: 1 } })],
  ...
  ["reset_board_credentials", act("reset_board_credentials", { boardKey: ids.fontCollectionBoardKey })],
  ["reset_board_preview", act("reset_board_preview", { boardKey: ids.fontCollectionBoardKey })],
  ["reset_board_editor_data", act("reset_board_editor_data", { boardKey: BOARD })],

  ["add_variant", act("add_variant", { boardKey: BOARD })],

  // Node property writes and resets.
  ["set_node_properties", act("set_node_properties", { nodeId: ids.uv1Id, properties: {} })],
  ["reset_node_property", act("reset_node_property", { nodeId: ids.uv1Id, propertyKey: ids.buttonPropKey })],
  ["reset_node", act("reset_node", { nodeId: ids.uv1Id })],
  ["set_node_state_properties", act("set_node_state_properties", { nodeId: ids.uv1Id, state: "hover", properties: {} })],
  ["reset_node_state_property", act("reset_node_state_property", { nodeId: ids.uv1Id, state: "hover", propertyKey: ids.buttonPropKey })],
  ["reset_node_state", act("reset_node_state", { nodeId: ids.uv1Id, state: "hover" })],

  // Custom-state registry.
  ["add_custom_state", act("add_custom_state", { key: "newstate", label: "New" })],
  ["remove_custom_state", act("remove_custom_state", { key: "mystate" })],
  ["rename_custom_state", act("rename_custom_state", { key: "mystate", label: "Renamed" })],

  // Paint layer stack.
  ["add_node_layer", act("add_node_layer", { nodeId: ids.uv1Id, property: "background", seed: {} })],
  ["remove_node_layer", act("remove_node_layer", { nodeId: ids.uv1Id, property: "background", index: 1 })],
  ["reorder_node_layer", act("reorder_node_layer", { nodeId: ids.uv1Id, property: "background", fromIndex: 0, toIndex: 1 })],
  ["set_node_layer_kind", act("set_node_layer_kind", { nodeId: ids.uv1Id, property: "background", layerIndex: 0, kind: "color" })],

  // Node misc writes and resets.
  ["set_node_label", act("set_node_label", { nodeId: ids.uv1Id, label: "ZZZ-Label" })],
  ["set_node_ref", act("set_node_ref", { nodeId: ids.uv1Id, ref: "myref" })],
  ["set_node_theme", act("set_node_theme", { nodeId: ids.uv1Id, theme: null })],
  ["set_node_editor_data", act("set_node_editor_data", { nodeId: ids.uv1Id, editorData: { x: 1 } })],
  ["set_node_repeat", act("set_node_repeat", { nodeId: ids.uv1Id, repeat: undefined })],
  ["reset_node_label", act("reset_node_label", { nodeId: ids.uv1Id })],
  ["reset_node_editor_data", act("reset_node_editor_data", { nodeId: ids.uv1Id })],

  // Theme writes.
  ["set_theme_label", act("set_theme_label", { themeId: THEME_VARIANT_ID, label: "Copy" })],
  ["set_theme_editor_data", act("set_theme_editor_data", { themeId: THEME_VARIANT_ID, editorData: { x: 1 } })],
  ["set_theme_override", act("set_theme_override", { themeId: THEME_VARIANT_ID, path: "color.primary", value: null })],
  [
  ...
  ["set_theme_custom_token_name", act("set_theme_custom_token_name", { themeId: THEME_VARIANT_ID, section: "swatch", key: "custom1", name: "Renamed Swatch" })],
  ...
  ["reset_variant_to_catalog", act("reset_variant_to_catalog", { variantRootId: ids.uv1Id })],
  ["reset_instance_to_source", act("reset_instance_to_source", { instanceId: ids.uv1ChildId })],
  ["reset_instance_to_original", act("reset_instance_to_original", { instanceId: ids.uv1ChildId })],
  ["reset_default_variant_to_catalog", act("reset_default_variant_to_catalog", { defaultVariantRootId: ids.defaultRootId })],
  ["reset_component_to_catalog", act("reset_component_to_catalog", { boardKey: BOARD })],

  // Move and insert.
  ["move_instance", act("move_instance", { instanceId: ids.uv1ChildId, target: { parentId: ids.uv1Id, index: 0 } })],
  ["move_instance_directional", act("move_instance_directional", { instanceId: ids.uv1ChildId, direction: "forward" })],
  ["reorder_instance_in_parent", act("reorder_instance_in_parent", { instanceId: ids.uv1ChildId, newIndex: 0 })],
  ["insert_variant_instance", act("insert_variant_instance", { variantId: ids.uv2Id, target: { parentId: ids.uv1Id } })],
  ["insert_duplicate_instance", act("insert_duplicate_instance", { instanceId: ids.uv1ChildId, target: { parentId: ids.uv2Id } })],
  ["insert_default_instance", act("insert_default_instance", { boardKey: BOARD, parentId: ids.uv1Id })],

  // Theme resets.
  ["reset_theme_tokens", act("reset_theme_tokens", { themeId: THEME_VARIANT_ID })],
  ["reset_theme_label", act("reset_theme_label", { themeId: THEME_VARIANT_ID })],
  ["reset_theme_editor_data", act("reset_theme_editor_data", { themeId: THEME_VARIANT_ID })],
  ["reset_theme_override", act("reset_theme_override", { themeId: THEME_VARIANT_ID, path: "color.primary" })],
  ...
  ["duplicate_theme", act("duplicate_theme", { themeId: ids.themeDefaultId, newThemeId: "dup-theme-2" })],

  // Font collections.
  ["set_font_collection_label", act("set_font_collection_label", { fontCollectionId: FC_VARIANT_ID, label: "FC" })],
  ["set_font_collection_editor_data", act("set_font_collection_editor_data", { fontCollectionId: FC_VARIANT_ID, editorData: { x: 1 } })],
  ["set_font_collection_override", act("set_font_collection_override", { fontCollectionId: FC_VARIANT_ID, path: "families.primary", value: null })],
  ["set_font_collection_family_variant", act("set_font_collection_family_variant", { fontCollectionId: FC_VARIANT_ID, slot: "primary", variant: "regular", enabled: true })],
  ["set_font_collection_family_preset", act("set_font_collection_family_preset", { fontCollectionId: FC_VARIANT_ID, slot: "primary", preset: "none" })],
  ["reset_font_collection_label", act("reset_font_collection_label", { fontCollectionId: FC_VARIANT_ID })],
  ["reset_font_collection_editor_data", act("reset_font_collection_editor_data", { fontCollectionId: FC_VARIANT_ID })],
  ["reset_font_collection_override", act("reset_font_collection_override", { fontCollectionId: FC_VARIANT_ID, path: "families.primary" })],
  ["reset_font_collection", act("reset_font_collection", { fontCollectionId: FC_VARIANT_ID })],
  ["delete_font_collection", act("delete_font_collection", { fontCollectionId: FC_VARIANT_ID })],
  ["duplicate_font_collection", act("duplicate_font_collection", { fontCollectionId: ids.fcDefaultId, newFontCollectionId: "dup-fc-2" })],
  ["add_font_collection_custom_family", act("add_font_collection_custom_family", { fontCollectionId: FC_VARIANT_ID, name: "Fam", variants: ["regular"] })],
  ["remove_font_collection_custom_family", act("remove_font_collection_custom_family", { fontCollectionId: FC_VARIANT_ID, key: "custom1" })],

  // Icon sets.
  ["set_icon_set_label", act("set_icon_set_label", { iconSetId: IS_VARIANT_ID, label: "IS" })],
  ["set_icon_set_override", act("set_icon_set_override", { iconSetId: IS_VARIANT_ID, path: "includedIcons.seldon-lines", value: true })],
  ["reset_icon_set_override", act("reset_icon_set_override", { iconSetId: IS_VARIANT_ID, path: "includedIcons.seldon-lines" })],
  ["reset_icon_set", act("reset_icon_set", { iconSetId: IS_VARIANT_ID })],
  ["set_icon_set_subcategory_preset", act("set_icon_set_subcategory_preset", { iconSetId: IS_VARIANT_ID, subcategory: "user-interface/text", preset: "none" })],
  ["delete_icon_set", act("delete_icon_set", { iconSetId: IS_VARIANT_ID })],
  ["duplicate_icon_set", act("duplicate_icon_set", { iconSetId: ids.isDefaultId, newIconSetId: "dup-is-2" })],

  // Reserved stub maps and transcript no-ops.
  ["stubs_add_font_collection_row", act("stubs_add_font_collection_row", {})],
  ["stubs_remove_font_collection_row", act("stubs_remove_font_collection_row", {})],
  ["stubs_set_font_collection_field", act("stubs_set_font_collection_field", {})],
  ["stubs_duplicate_font_collection_row", act("stubs_duplicate_font_collection_row", {})],
  ...
  ["transcript_add_message", act("transcript_add_message", { chatMessage: "hi", expectUserAnswer: false })],
]

describe("workspaceReducer exhaustive dispatch", () => {
  it.each(CASES)("routes %s through the middleware stack", (_name, action, ws) => {
