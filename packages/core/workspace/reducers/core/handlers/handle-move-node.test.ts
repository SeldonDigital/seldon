...
import { handleMoveNode } from "./handle-move-node"

describe("handleMoveNode", () => {
  it("should move a child node within the same variant hierarchy", () => {
  ...
  it("should not move a variant node (only instances can be moved)", () => {
    const result = handleMoveNode(
      {
        nodeId: "variant-button-default" as any,
        target: {
