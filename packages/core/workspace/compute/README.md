...
| Function | File | Core Purpose | Use |
| --- | --- | --- | --- |
...
| `getEffectiveNodeProperties` | `compute-node-properties.ts` | Merges schema defaults, template chains, and overrides. | Called when selectors and editor bridges need a node or board property snapshot. |
| `computeNodeProperties` | `compute-node-properties.ts` | Returns effective or computed properties for a node or board. | Called by editor and preview code that need workspace-aware property output. |
| `getNodes` | `compute-node-properties.ts` | Reads the active workspace node map. | Used by compute helpers that support current and transitional workspace shapes. |
...
| `mergeLayerArrays` | `compute-node-properties.ts` | Merges layered paint arrays by slot. | Used by property merging for background, gradient, and shadow stacks. |
