...
export function formatEntryId(prefix: string, boardKey: string, suffix: string): string {
  return `${prefix}-${boardKey}-${suffix}`
}

/** Derives the owning board key from an entry id, or null when the id does not match the prefix. */
export function boardKeyFromEntryId(prefix: string, entryId: string): string | null {
  const start = `${prefix}-`
