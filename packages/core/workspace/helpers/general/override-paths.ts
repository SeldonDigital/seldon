...
export function getOverrideAtPath(target: Record<string, unknown>, path: string): unknown {
...
export function deleteOverrideAtPath(target: Record<string, unknown>, path: string): void {
  const keys = path.split(".").filter(Boolean)
