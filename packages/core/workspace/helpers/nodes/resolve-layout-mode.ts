 ...
 * `"flexbox"`.
 */
export function resolveLayoutMode(node: EntryNode, workspace: Workspace): LayoutMode {
  const catalogId = getNodeCatalogId(node, workspace)

  if (!catalogId || !isComponentId(catalogId)) return "flexbox"

  return getComponentSchema(catalogId).layout === ComponentLayout.GRID ? "grid" : "flexbox"
}
