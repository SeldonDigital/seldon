...
| `WORKSPACE_EDITABLE_THEME_ENTRY_ID` | `workspace-editable-theme.ts` | Constant id `theme-default-default` for the editable theme row. \| The stock default theme entry. |
| `ensureWorkspaceEditableThemeEntry` | `workspace-editable-theme.ts` | Adds the `theme-default-default` row when missing. \| Used by `set_workspace` and creation paths. |
