...
import { DEFAULT_THEME_BOARD_AUTHOR, getDefaultBoardLabel } from "./default-board-metadata"
import { componentBoardDefaultNodeId, componentBoardSchemaVariantNodeId } from "./entry-node-ids"
...
describe("isBoard", () => {
  it("distinguishes catalog rows from node entries", () => {
    ...
    expect(isBoard({ id: "n", type: "instance" } as unknown as EntryNode)).toBe(false)
  })
})
...
describe("default-board-metadata", () => {
  it("exposes the default author and a board label", () => {
    ...
    const ws = componentWorkspace()
    const label = getDefaultBoardLabel(ComponentId.BUTTON, ws.boards[ComponentId.BUTTON])
