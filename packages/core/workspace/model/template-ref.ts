...
export function parseNodeLink(value: string): Extract<ParsedNodeTemplate, { kind: "node" }> | null {
  const parsed = parseNodeTemplate(value)
...
function parseFontCollectionTemplate(value: string): ParsedFontCollectionTemplate | null {
  const parts = splitTemplateRef(value)
...
export function formatFontCollectionCatalog(fontCollectionCatalogId: string): string {
...
export function getFontCollectionTemplateCatalogId(value: string): string | null {
  const parsed = parseFontCollectionTemplate(value)

  return parsed?.kind === "catalog" ? parsed.fontCollectionCatalogId : null
}

export function getFontCollectionTemplateFontCollectionId(value: string): string | null {
  const parsed = parseFontCollectionTemplate(value)
