...
import { isEntryFontCollectionDefault, isEntryFontCollectionVariant } from "./entry-font-collection"
...
describe("font-collection entry guards", () => {
  it("classifies every seeded font-collection entry by type", () => {
    ...
    for (const collection of collections) {
      expect(isEntryFontCollectionDefault(collection)).toBe(collection.type === "default")
      expect(isEntryFontCollectionVariant(collection)).toBe(collection.type === "variant")
    }
  })
})
