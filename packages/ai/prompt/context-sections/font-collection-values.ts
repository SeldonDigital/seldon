...
export function fontCollectionValuesSection(
  ...
  const collection = workspaceFontCollectionService.getFontCollection(fontCollectionId, workspace)

  if (!collection) return []
  const selection = workspaceFontCollectionService.getVariantSelection(fontCollectionId, workspace)
  ...
  for (const [slot, family] of Object.entries(collection.families)) {
    ...
    const weights = sortFontVariants(variants)
      .map((variant) => `${variant}=${isVariantEnabled(slotSelection, variant) ? "on" : "off"}`)
      .join(", ")
