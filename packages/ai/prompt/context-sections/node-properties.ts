 ...
 */
export function nodePropertiesSection(workspace: Workspace, nodeId: string): string[] {
  ...
  try {
    effective = getEffectiveProperties(nodeId, workspace) as Record<string, unknown>
  ...
  for (const key of keys) {
    lines.push(`- ${key} = ${summarizeValue(effective[key])} (${status[key] ?? "inherited"})`)
  }
