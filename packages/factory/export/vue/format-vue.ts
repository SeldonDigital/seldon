 ...
 * style block in one pass. An import-sort plugin applies only when the
 * destination Prettier config loads one.
 *
 * Prettier ends its output with a newline, so a caller adds none.
 */
export async function formatVue(content: string, options?: ExportFormatContext) {
  if (options?.skipFormat) {
    return content
  }

  const firstPass = await formatWithPrettier(content, { parser: "vue" }, options)
  ...
  return formatWithPrettier(firstPass, { parser: "vue" }, options)
}
