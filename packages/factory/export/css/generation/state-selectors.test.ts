...
describe("getStateSelectorSuffixes", () => {
  ...
  it("maps multi-suffix reserved states", () => {
    expect(getStateSelectorSuffixes("disabled")).toEqual([":disabled", '[aria-disabled="true"]'])
  })

  it("maps attribute-backed reserved states", () => {
    expect(getStateSelectorSuffixes("error")).toEqual(['[aria-invalid="true"]'])
    expect(getStateSelectorSuffixes("selected")).toEqual(['[aria-selected="true"]'])
  })
  ...
  it("maps a custom state to a runtime-toggled class", () => {
    expect(getStateSelectorSuffixes("myState")).toEqual([`.${STATE_CLASS_PREFIX}myState`])
  })
})
...
describe("getAncestorStateSelectorSuffixes", () => {
  ...
  it("uses :has(:checked) for checked on an ancestor", () => {
    expect(getAncestorStateSelectorSuffixes("checked")).toEqual([":has(:checked)"])
  })
  ...
  it("maps a custom state to the same runtime-toggled class", () => {
    expect(getAncestorStateSelectorSuffixes("myState")).toEqual([`.${STATE_CLASS_PREFIX}myState`])
  })
})
