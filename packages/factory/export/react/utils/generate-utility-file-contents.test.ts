...
describe("getUtilityFileContents", () => {
  ...
  it("exports the resize helpers from the resize file", () => {
    const file = getUtilityFileContents(options).find((f) => f.path.endsWith("resize.ts"))
  ...
  it("exports the icon registry helpers from the icon-registry file", () => {
    const file = getUtilityFileContents(options).find((f) => f.path.endsWith("icon-registry.ts"))
  ...
  it("exports combineClassNames from the class-name file", () => {
    const file = getUtilityFileContents(options).find((f) => f.path.endsWith("class-name.ts"))
