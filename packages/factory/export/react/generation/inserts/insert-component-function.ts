...
export function insertComponentFunction(
  ...
  // React takes `ref` as a plain prop, so a caller's ref rides the `{...props}`
  // spread onto whichever root element gets it. No wrapper is needed.
  const functionSource = `export function ${componentName}(${propsSpread}: ${interfaceName}) {
      ${declarations}
      ${returns}
    }`
  ...
  return transformSource({
    ...
    source,
    content: `
    ${generateJSDocComment(component, workspace, propNames)}
    ${functionSource}`,
  })
}
