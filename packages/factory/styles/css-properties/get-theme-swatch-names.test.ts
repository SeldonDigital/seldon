...
import { getThemeSwatchVarNames, getThemeSwatchVarReference } from "./get-theme-swatch-names"
...
describe("getThemeSwatchVarReference", () => {
  it("returns undefined for keys that are not swatch references", () => {
    expect(getThemeSwatchVarReference("@font.body", defaultTheme)).toBeUndefined()
  })

  it("returns undefined for a swatch id missing from the theme", () => {
    expect(getThemeSwatchVarReference("@swatch.doesNotExist", defaultTheme)).toBeUndefined()
  })
