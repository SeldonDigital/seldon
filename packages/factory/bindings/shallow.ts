...
import type { BindingsConfig, BindingsManifest, FileSource, RefConsumer } from "./types"
 ...
 */
export async function scanBindingsShallow(
  source: FileSource,
 ...
 */
function readFileRefs(path: string, text: string): FoundRef[] {
 ...
 * memoizes the map.
 */
function findMapDeclaration(text: string, name: string): number {
...
function readPropKeys(segment: string): string[] {
 ...
 * stepped over whole.
 */
function splitObjectEntries(text: string, open: number): Segment[] {
...
function readKey(segment: string): string | null {
...
function getLine(text: string, index: number): number {
  let line = 1
