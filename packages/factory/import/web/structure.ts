 ...
 * piece contains.
 */
export function describeTree(node: FunctionalNode, maxDepth = 3, depth = 0): string {
  const indent = "  ".repeat(depth)
  const role = node.role ? ` role=${node.role}` : ""
  const classes = node.evidence.classes?.length ? ` class="${node.evidence.classes.join(" ")}"` : ""
  ...
  const kids = node.children.map((child) => describeTree(child, maxDepth, depth + 1)).join("\n")
