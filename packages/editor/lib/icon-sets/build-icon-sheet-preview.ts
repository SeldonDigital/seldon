...
/**
 * Builds a throwaway workspace containing an Avatar board and its nodes.
 *
 * The result is deterministic, so it is instantiated once and cached. Icon set
 * board previews clone this base, point the iconic Avatar root at a board theme,
 * and inject one icon symbol per included icon. The root is the iconic Avatar
 * variant node (`component-avatar-icon`).
 */
export function getIconSheetPreviewBase(): IconSheetPreviewBase {
  ...
  const rootId = componentBoardSchemaVariantNodeId(ComponentId.AVATAR, "icon")
