...
export function useBoardStateMenu(): ComputedRef<BoardStateMenu> {
  ...
  return computed<BoardStateMenu>(() => {
    ...
    const activeState = boardKey ? boardStateStore.getActiveState(boardKey) : NORMAL_STATE
    ...
    if (board) {
      const addStatesFromChain = (startId: string, target: Set<NodeState>): void => {
      ...
      walkComponentTree(board, (ref, parent) => {
        addStatesFromChain(ref.id, parent === null ? ownOverrideStates : childOverrideStates)
      })
    }
