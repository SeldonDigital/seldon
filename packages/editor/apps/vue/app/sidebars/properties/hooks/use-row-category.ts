...
export function useRowCategory(section: ComputedRef<PropertySection>) {
  ...
  const isExpanded = computed(() => expansion.isCategoryExpanded(section.value.category))
  ...
  const icon = computed(() => (isExpanded.value ? "material-unfoldLess" : "material-unfoldMore"))
