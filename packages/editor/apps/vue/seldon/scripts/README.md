...
This export baked in `vue` and `seldon`. The framework takes no flag, because
this export emitted only the front ends a vue project reaches. Re-export to
change it.
...
The full scan needs `typescript` and `@vue/compiler-sfc`,
