...
The editable source is `.seldon/seldon-editor.vue.json` in your repo. The name is the
...
```sh
npx seldon-export --input .seldon/seldon-editor.vue.json --platform vue
```

Commit `.seldon/seldon-editor.vue.json` as the design source, and ignore the Editor's
...
{
  "scripts": {
    "seldon:export": "seldon-export --input .seldon/seldon-editor.vue.json --platform vue"
  }
}
```
