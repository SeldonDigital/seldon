...
export interface PanelTokenPreviewProps extends HTMLAttributes<HTMLElement> {
...
//
const sdn: PanelTokenPreviewProps = {
  role: "dialog",
  "aria-hidden": "false",
  chipAssist: {
 ...
 * ```
 */
export function PanelTokenPreview({
  className = "",
  chipAssist,
  textLabel,
  icon,
  ...
  seldonRefs,
  ...props
}: PanelTokenPreviewProps) {
  const panelTokenPreviewClassName = combineClassNames("sdn-panel-token-preview", className)
  ...
  return (
    <HTMLDiv
      className={panelTokenPreviewClassName}
      role={sdn["role"]}
