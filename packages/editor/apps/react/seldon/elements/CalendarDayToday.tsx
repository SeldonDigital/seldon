...
export interface CalendarDayTodayProps extends HTMLAttributes<HTMLElement> {
...
//
const sdn: CalendarDayTodayProps = {
  wrapperElement: "div",
  "aria-hidden": "false",
  "aria-current": "date",
  textLabel: {
 ...
 * ```
 */
export function CalendarDayToday({
  className = "",
  ...
  const calendarDayTodayClassName = combineClassNames("sdn-calendar-day-today", className)

  const textLabelProps = mergeOptionalSlot(sdn.textLabel, textLabel, seldonRefs)

  return (
    <Frame
      className={calendarDayTodayClassName}
      aria-hidden={sdn["aria-hidden"]}
      aria-current={sdn["aria-current"]}
      {...props}
    >
      {children !== undefined ? (
        children
