...
/**
 * The hidden chips the column's metrics are measured from. Each is drawn at the width
 * its own label asks for, so the widest reports the width every chip then takes.
 */
export const refChipMeasureStyle: CSSProperties = {
  position: "absolute",
  top: 0,
  left: 0,
  display: "grid",
...
/**
 * The chip's own box, held at the width the column shares. The schema draws it at
 ...
 */
export function refChipBoxStyle(width: number): CSSProperties {
  return {
    width: `${width}px`,
