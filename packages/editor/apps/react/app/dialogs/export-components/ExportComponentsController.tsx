...
function ExportComponentsDialog({
  ...
  const slots = useMemo<Partial<DialogExportComponentProps>>(
    () => ({
      ...
      // Every slot on a menu row is opt-in, so each bullet keeps a positional
      // enabler beside its row. A ref alone reaches a slot that already renders;
      // it cannot bring back one the row leaves off.
