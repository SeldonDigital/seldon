 ...
 *
 * Facets compose across the node and its ancestor chain: a row is dimmed or
 * italic when any collected state belongs to the matching set. The resulting
 * matrix for a node's own state is:
 *
 * - `SHOW`: normal
 * - `HIDE`: italic
 * - `STUB`: normal
 ...
 *
 * Dimmed rows read as gray through the leaf `[aria-disabled]` style.
 */
