 ...
 */
export function FramerExpandable({ isExpanded, children }: FramerExpandableProps) {
  const panel = isExpanded ? (
    <motion.div initial={collapsed} animate={expanded} exit={collapsed} transition={transition}>
      {children}
    </motion.div>
  ) : null
