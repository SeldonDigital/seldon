...
import { useOrdinalPreviewStore, useOrdinalSelection } from "../hooks/use-ordinal-preview-store"
...
// Fallback board layout: variants stacked in a column. The board's own component
// properties (orientation, align, margin, padding, gap, wrap, clip, background)
// are resolved to CSS and spread over this, so any LAYOUT control the user sets
// wins while an unset property keeps this default. `position` and `minHeight`
// are canvas-only and always stay.
...
export function ThemeBoard({ board }: ThemeBoardProps) {
  ...
  const boardVariantIds = isThemeBoard(board) ? board.variants.map((variant) => variant.id) : []
...
function buildChipTextContent(
  ...
  for (const node of Object.values(previewBase.nodes)) {
    if (getNodeCatalogComponentId(node, previewBase) !== ComponentId.COLOR_CHIP) {
      continue
    }
...
function buildChipStyleOverrides(
  ...
  const marginValue = themeOrdinalValue(ordinalStepRef("margin", stepByScale.margin))
  const paddingValue = themeOrdinalValue(ordinalStepRef("padding", stepByScale.padding))
  const gapValue = themeOrdinalValue(ordinalStepRef("gap", stepByScale.gap))
  const borderValue = themeOrdinalValue(ordinalStepRef("border", stepByScale.border))
  const cornersValue = themeOrdinalValue(ordinalStepRef("corners", stepByScale.corners))
  ...
  for (const node of Object.values(previewBase.nodes)) {
    if (getNodeCatalogComponentId(node, previewBase) === ComponentId.ORDINAL_CHIP) {
 ...
 */
function ThemeVariantPreview({ variantEntryId, themes }: ThemeVariantPreviewProps) {
  ...
  const contentById = useMemo(() => {
    const content: Record<string, string> = buildChipTextContent(previewBase, theme)

    for (const button of layout.legendButtons) {
      content[button.labelNodeId] = ordinalStepName(button.scale, stepByScale[button.scale], theme)
    }

    for (const row of layout.chipRows) {
      content[row.textNodeId] = ordinalStepValueText(row.scale, stepByScale[row.scale], theme)
    }
  ...
  const handlePreviewClick = useCallback(
    (event: MouseEvent<HTMLDivElement>) => {
      let element = (event.target as HTMLElement).closest<HTMLElement>("[data-preview-node-id]")
      ...
      while (element) {
        ...
        element = element.parentElement?.closest<HTMLElement>("[data-preview-node-id]") ?? null
      }
    },
    [scaleByButtonId],
  )
  ...
  return (
    ...
    >
      ...
      <MenuController open={menuOpen} anchorRef={anchorRef} onClose={closeMenu} items={menuItems} />
    </PreviewItemWrapper>
  )
}
