...
import { useSelection, useStore as useSelectionStore } from "@app/workspace/hooks/use-selection"
...
function getPanCursor(isPanning: boolean, isSpacebarPressed: boolean): CSSProperties["cursor"] {
...
export const Canvas = () => {
  ...
  useEffect(() => {
    ...
    const enteringComponent = activeTool === "component" && prevTool !== "component"
    const leavingComponent = activeTool !== "component" && prevTool === "component"

    if (enteringComponent) {
      const { selectedNodeId, selectedNodeRootId } = useSelectionStore.getState()
...
const CanvasContainer = () => {
  ...
  const backdropColor =
    resolvedMode === "dark" ? "var(--sdn-swatch-background)" : "var(--sdn-swatch-foreground)"
