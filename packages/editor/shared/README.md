...
Folder export runs through the local export route. [lib/export/run-local-export.ts](./lib/export/run-local-export.ts) posts the workspace to `/api/export`, which [vite/export-api-plugin.ts](./vite/export-api-plugin.ts) serves by bundling the Factory export handler. The app then writes the returned files to the folder the browser picks. `.seldon` stays at that project root. Generated files nest under `exportSettings.outputFolder` when that field is set. A bound project writes the workspace source under `.seldon` and a hashed backup under `.seldon/workspaces`. MCP and the CLI use that same source file.
