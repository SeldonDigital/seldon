 ...
 * node id is shared both across variant columns and across sibling copies inside
 * one column, so the full path is what uniquely identifies the clicked or
 * hovered copy. Both the canvas and the objects sidebar stamp this path on their
 * selectable elements, so one code path resolves selection for both.
 */
 ...
 * nearest element tagged with `data-selection-id` / `data-selection-kind`. Pure
