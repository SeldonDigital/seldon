 ...
 */
const MATCH_SIBLING_FACETS: Record<string, { colorKey: string; kind: "brightness" | "opacity" }> =
  Object.fromEntries(
...
export function resolveMatchSiblingLock(
  ...
  const enabled =
    sibling.kind === "brightness" ? !!params?.includeBrightness : !!params?.includeOpacity
  ...
  try {
    const computed = computeNodeProperties(getPropertiesSubjectId(node), workspace as never)
