...
export function flattenShownBorderSides(
  ...
  for (const side of BORDER_SIDE_KEYS) {
    ...
    const value = resolvePropertyValueForDisplay(mergedProperties, side) || EMPTY_VALUE
    const status = propertyStatus[side] || "unset"

    out.push(createFlatProperty(side, value, status, node, workspace, theme, state))
    out.push(
      ...getSubProperties(side, value, workspace, node, propertyStatus, theme, mergedProperties),
    )
  }
