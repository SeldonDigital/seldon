...
import { THEME_TOKEN_SECTIONS, getAllThemeTokenSchemas } from "@seldon/core/themes/schemas"
import {
  CALCULATED_SWATCHES,
...
const SECTION_LABELS = new Map(THEME_TOKEN_SECTIONS.map((section) => [section.id, section.label]))
...
function buildControl(
  ...
  if (
    ...
    const matching = schema.options.find((option) => String(option.value) === String(value))
...
export function buildThemeTokenRows(
  ...
  for (const schema of getAllThemeTokenSchemas(theme)) {
    ...
    const { control, isDimmed, iconColorValue } = buildControl(schema, value, theme)
...
export function groupThemeTokenRows(rows: ThemeTokenRow[]): ThemeTokenSectionRows[] {
