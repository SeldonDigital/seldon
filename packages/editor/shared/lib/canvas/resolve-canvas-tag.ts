 ...
 * nodes to plain DOM tags, so both editors can paint identical markup from one
 * config.
 */
 ...
 * selection. Icons render through the shared Icon layer, so they return a
 * dedicated marker.
 */
export function resolveCanvasTag(
  ...
  if (returns === "custom") {
    // Custom templates build their own internal DOM. A DOM canvas renders a
