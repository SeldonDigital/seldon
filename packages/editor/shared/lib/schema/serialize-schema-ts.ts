...
function isStringEnumObject(value: unknown): value is Record<string, string> {
  ...
  return (
    entries.length > 0 &&
    entries.every(([key, member]) => typeof member === "string" && IDENTIFIER_KEY.test(key))
  )
}

/** `ValueType` string -> `Sdn.ValueType.{MEMBER}` reference. */
const valueTypeRefByValue = new Map<string, string>(
  Object.entries(Sdn.ValueType).map(([member, value]) => [value, `Sdn.ValueType.${member}`]),
)
...
const unitRefByValue = new Map<string, string>(
  Object.entries(Sdn.Unit).map(([member, value]) => [value, `Sdn.Unit.${member}`]),
)
...
function writeString(value: string, path: string[], context: ValueContext): string {
...
function writeArray(values: unknown[], depth: number, path: string[]): string {
  ...
  const lines = values.map((value) => `${inner}${writeValue(value, depth + 1, path)},`)

  return `[\n${lines.join("\n")}\n${INDENT.repeat(depth)}]`
}

function writeObject(value: Record<string, unknown>, depth: number, path: string[]): string {
  const entries = Object.entries(value).filter(([, entryValue]) => entryValue !== undefined)

  if (!entries.length) return "{}"

  const tagged = isTaggedValue(value)
  const isOption = tagged && (value as { type: string }).type === Sdn.ValueType.OPTION
  const isComputed = tagged && (value as { type: string }).type === Sdn.ValueType.COMPUTED
  const inner = INDENT.repeat(depth + 1)
    ...
    if (tagged && entryKey === "type") {
      ...
      written =
        typeof entryValue === "string"
          ? (computedFunctionRefByValue.get(entryValue) ?? JSON.stringify(entryValue))
